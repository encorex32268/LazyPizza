...
import com.lihan.lazypizza.cart.presentation.order_checkout.components.OrderPlacedSection
...
fun OrderCheckoutRoot(
    ...
    LaunchedEffect(sharedState.items,state.isSentOrder){
        if (sharedState.items.isEmpty() && !state.isSentOrder){
            onBack()
        }
    }
    ...
    OrderCheckoutScreen(
        ...
        onAction = { action ->
            when(action){
                ...
                OrderCheckoutAction.OnNavigateToMenu -> onNavigateToMenu()
                else -> Unit
            }
        ...
        if (state.isSentOrder){
            Box(
                ...
                OrderPlacedSection(
                    ...
                    pickUpTime = state.placedPickUpTime?.asString()?:"",
                    onNavigateToMenu = {
                        onAction(OrderCheckoutAction.OnNavigateToMenu)
                    }
                )
            }
        }else{
                ...
                Column(
                    ...
                    AppButton(
                        ...
                        onClick = {
                            ...
                            val pickupTime = state.pickUpTime?.asString(context)
                            onAction(OrderCheckoutAction.OnPlaceOrderClick(
                                cartItems = sharedState.items,
                                pickupTime = if (state.pickUpTimeType == PickUpTimeType.EarliestAvailableTime) pickupTime else null
                             ))
                        }
                    )
                }
